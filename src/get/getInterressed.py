...
def getInterressed(id, lastReqId):
    redis = RedisIface()
    size = 40960
    try:
        json_object = {}
        ...
        if id == None:
            del redis
            logger.critical("proxy getInterressed id error")
            return {}
        req_result = redis.redis_hget(id, 'requestsOfInterressed')
...
def getInterressedEntry():
    try:
        request = flask.request
        id = request.args.get('id')
        lastReqId = request.args.get('lastReqId')
        return getInterressed(id, lastReqId)
    except Exception as e:
        logger.critical("proxy getInterressedEntry args error: " + str(e))
        return {}