...
def send_confirmation_email(user_email, confirmation_link):
    ...
    body = f'Cliquez sur le lien suivant pour confirmer votre inscription : {confirmation_link}'
    # Créer l'e-mail
    msg = MIMEMultipart()
    msg['From'] = from_email
    msg['To'] = user_email
...
def add_play_logo_to_thumbnail(thumbnail_path, output_path, logo_size=50):
    try:
        ...
        circle_radius = logo_size + 5  # Outer circle radius slightly larger than the triangle
        ...
        triangle_overlay = Image.new("RGBA", thumbnail.size, (255, 255, 255, 0))
...
def create_video_thumbnail(video_path, thumbnail_path, time="00:00:01"):
    try:
        (
            ...
            .filter('scale', 320, -1)    # Resize the frame to a smaller size (optional)
...
def mediaType(ext):
    audio = {'mp3', 'wav', 'aac', 'flac', 'ogg', 'wma', 'm4a', 'aiff', 'alac', 'opus', 'amr', 'mid', 'midi', 'pcm', 'ra', 'mka'}
    video = {'mp4', 'avi', 'mkv', 'mov', 'wmv', 'flv', 'webm', 'mpeg', 'mpg', 'm4v', '3gp', 'rm', 'rmvb', 'vob', 'ts', 'asf', 'f4v', 'divx'}
    image = {'jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff', 'tif', 'svg', 'webp', 'heic', 'heif', 'raw', 'psd', 'ai', 'eps', 'ico', 'dds', 'jfif', 'jp2'}
    # TODO add emoji
    if ext in image:
        return "image"
    if ext in audio:
